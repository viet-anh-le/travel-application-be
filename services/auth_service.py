...
class AuthService:
    async def get_me(self, user_id: str):
        ...
        return OkResponse(
            "Lấy thông tin người dùng thành công", user.model_dump(exclude={"password"})
        )
    ...
    async def google_login(self, token: str):
        ...
        found_user = await User.find_one({"$or": [{"email": email}, {"google_id": sub}]})
        ...
        access_token = generate_jwt(token_payload, "2d")
    ...
    async def refresh_token(self, refresh_token: str):
        try:
            # 1. Giải mã và verify Refresh Token (Check hạn, check chữ ký)
            payload = jwt.decode(refresh_token, SECRET_KEY, algorithms=["HS256"])

            # (Tùy chọn) Kiểm tra xem đây có đúng là refresh token không
            # nếu bạn có lưu type="refresh" vào payload lúc tạo

            user_id = payload.get("id")
            if not user_id:
                raise BadRequestError("Token không hợp lệ (Missing ID)")

        except JWTError:
            raise BadRequestError("Refresh token đã hết hạn hoặc không hợp lệ")
        ...
        new_access_token = generate_jwt(token_payload, "2d")
        new_refresh_token = generate_jwt(token_payload, "7d")
    ...
    async def forgot_password(self, payload: dict):
        ...
        return OkResponse(
            "OTP sent to your email", {"user_id": str(user.id), "expireIn": expire_in}
        )
    ...
    async def resend_otp(self, payload: dict):
        ...
        return OkResponse(
            "OTP resent to your email", {"user_id": str(user.id), "expireIn": expire_in}
        )
    ...
    async def google_calendar_authenticate(self, user_id: str):
        ...
        if (
            found_user.google_calendar
            and found_user.google_calendar.access_token
            and found_user.google_calendar.expiry_date
        ):
