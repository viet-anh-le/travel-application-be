...
async def detect_topic(payload: AskRequest):
    ...
    return {
        "topic": "Plan" if is_planning else "Other",
        "location": location,
        "raw_topic": topic
    }
...
async def create_schedule(
    ...
    if location is None:
        await chat_repository.save_message(session_id=session_id, message=ChatMessage(content=message, role="human"))
    ...
    if topic is None or "Plan" not in topic:
        await chat_repository.save_message(session_id=session_id, message=ChatMessage(content=message, role="human"))
        ai_message = await chat_repository.save_message(session_id=session_id, message=ChatMessage(content="Yêu cầu của bạn không liên quan đến việc lập kế hoạch du lịch. Vui lòng gửi yêu cầu khác.", role="ai"))
    ...
    agent_service = AgentService(chat_repository=chat_repository, retriever=parent_document_retriever, user_id=user_id)
            ...
            if final_answer_data:
                ...
                try:
                    ...
                    if trip_id and await request.is_disconnected():
                        ...
                        ai_msg = final_answer_data.get("ai_message_for_history") or final_answer_data["data"]
                        await chat_repository.save_message(session_id=session_id, message=ChatMessage(content=message, role="human"))
                        await chat_repository.save_message(session_id=session_id, message=ChatMessage(content=ai_msg, role="ai", trip_id=trip_id))
                except Exception as e:
...
async def ask(
    ...
    async def get_standalone():
        if len(chat_history) > 0:
            ...
            return res.get("standalone_question", message)
        return message
    
    standalone_question = await get_standalone()
    ...
    async def response_generator():
        ...
        try:
            ...
            await chat_repository.save_message(session_id=session_id, message=ChatMessage(content=message, role="human"))
            if full_ai_response:
                await chat_repository.save_message(session_id=session_id, message=ChatMessage(content=full_ai_response, role="ai"))
        except Exception as e:
            print(f"Streaming error: {e}")
            yield json.dumps({
                    "type": "error", 
                    "data": f"Lỗi hệ thống: {str(e)}"
                }) + "\n"
